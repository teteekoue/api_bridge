...
<head>
    ...
    <title>IA Bridge Pro v6.1</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="menu-items">
            <div id="m-chat" class="menu-item active" onclick="showSection('chat')">
                <i class="fas fa-comment-dots"></i> Mode Chat
            </div>
            <div id="m-config" class="menu-item" onclick="showSection('config')">
                <i class="fas fa-sliders-h"></i> Configuration
            </div>
            <div id="m-api" class="menu-item" onclick="showSection('api')">
                <i class="fas fa-link"></i> Endpoints API
            </div>
        </div>
        <div style="padding: 20px; font-size: 0.7rem; opacity: 0.4;">Pro Edition v6.1</div>
    </div>
    ...
    <div class="main-container">
        ...
        <section id="chat-section" class="content-section active">
            <div class="chat-header">
                ...
                <div>
                    <div style="font-weight: bold;">IA Assistant</div>
                    <div style="font-size: 0.75rem; color: #2e7d32;">● Système Prêt</div>
                </div>
            </div>
            <div class="messages" id="msgs">
                <div class="msg bot">Bonjour ! Sélectionnez un ou plusieurs fichiers et posez votre question. Je m'occupe du reste.</div>
            </div>
            <div class="input-container">
                <div id="filePreviews" class="file-previews"></div>
                <div class="input-wrapper">
                    <button class="action-btn btn-upload" onclick="document.getElementById('fileInput').click()" title="Joindre des fichiers">
        ...
        <section id="config-section" class="content-section">
            <div class="card">
                <h3>Réseau</h3>
                <div class="info-row"><span style="font-weight:600">Adresse IP</span><span>$ipAddress</span></div>
                <div class="info-row"><span style="font-weight:600">Port</span><span>$port</span></div>
                <div class="info-row"><span style="font-weight:600">Serveur</span><span class="status-badge">ACTIF</span></div>
            </div>
            <div class="card">
                <h3>Moteur d'Upload</h3>
                <div class="info-row"><span style="font-weight:600">Mode</span><span>Fallback (Catbox/Tmp/File.io)</span></div>
                <div class="info-row"><span style="font-weight:600">Fichiers Simultanés</span><span>Illimité</span></div>
            </div>
        </section>
        ...
        <section id="api-section" class="content-section">
            <div class="card">
                <h3>Documentation Intégration</h3>
                <label style="font-size:0.8rem; color:#666">ASK TEXT</label>
                <div class="copy-box">
                    <span id="u-ask">http://$ipAddress:$port/ask?q=...</span>
                    <i class="far fa-copy" onclick="copyText('u-ask')"></i>
                </div>
                <label style="font-size:0.8rem; color:#666; margin-top:15px; display:block">UPLOAD FILE</label>
                <div class="copy-box">
                    <span id="u-up">http://$ipAddress:$port/upload</span>
                    <i class="far fa-copy" onclick="copyText('u-up')"></i>
                </div>
            </div>
            ...
            try {
                ...
                if (filesQueue.length > 0) {
                    ...
                    for (let i = 0; i < filesQueue.length; i++) {
                        ...
                        loadingMsg.innerText = "Hébergement du fichier " + (i + 1) + "/" + filesQueue.length + " (" + f.name + ")...";
                        
                        const fd = new FormData();
                        fd.append('file', f);
                        
                        const upResp = await fetch(BASE + "/upload?file=" + encodeURIComponent(f.name), { method: 'POST', body: fd });
                        if (!upResp.ok) throw new Error("Échec upload " + f.name + ": " + await upResp.text());
                ...
                if (!askResp.ok) throw new Error("Erreur Automation: " + await askResp.text());
                ...
                loadingMsg.innerText = "Erreur technique lors du traitement.";
                debug.innerText = "DÉTAILS :\n" + e.message;
